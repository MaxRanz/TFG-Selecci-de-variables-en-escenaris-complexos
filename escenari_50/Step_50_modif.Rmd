...
resultats_step50 <- data.frame(
  N = integer(), pi = numeric(), correlation = numeric(),
...
system.time({
  for (i in 1:nrow(comb)) {
    ...
    # ── CANVI 1: iters segons N ──────────────────────────
    iters_i <- ifelse(N == 1000, 100, 25)
    # ────────────────────────────────────────────────────
    
    metrics <- list()
    
    # ── CANVI 2: matriu adaptada a iters_i ───────────────
    vars_selec_bin <- matrix(0, nrow = iters_i, ncol = 100)
    # ────────────────────────────────────────────────────

    for (j in 1:iters_i) {   # ── CANVI 3: usar iters_i ──
      print(paste("Escenari", i, "- Simulació", j, "/", iters_i,
    ...
    # ── CANVI 4: guardat parcial per no perdre feina ─────
...
save(resultats_step50, vars_seleccionades_step, file = "resultats_step50_modificat.RData")
```
