...
# Matrius de variables seleccionades
vars_seleccionades_step <- vector("list", nrow(comb))

temps_execucio <- system.time({
  ...
  for (i in 1:nrow(comb)) {
    ...
    # Matriu binària: simulacions × variables
    vars_selec_bin <- matrix(
      0,
      nrow = iters,
      ncol = num_vars
    )
    ...
    for (j in 1:iters) {
      ...
      vars_seleccionades <- which(
        names(coef(model_step)) != "(Intercept)"
      )

      # Guardar selecció binària
      if (length(vars_seleccionades) > 0) {
        vars_selec_bin[
          j,
          vars_seleccionades
        ] <- 1
      ...
      # Mètriques
      metrics[[j]] <- metriques(
        vars_seleccionades = vars_seleccionades,
        nv = num_vars
      )
    }

    # Resultats mitjans per escenari
    resultats_step50 <- rbind(
      resultats_step50,
      data.frame(
        N = N,
        pi = pi,
        correlation = correlation,
        Sensibilitat = mean(
          sapply(metrics, function(x) x["sensibilitat"]),
          na.rm = TRUE
        ),
        Especificitat = mean(
          sapply(metrics, function(x) x["especificitat"]),
          na.rm = TRUE
        ),
        VPP = mean(
          sapply(metrics, function(x) x["VPP"]),
          na.rm = TRUE
        ),
        VPN = mean(
          sapply(metrics, function(x) x["VPN"]),
          na.rm = TRUE
        ),
        Accuracy = mean(
          sapply(metrics, function(x) x["accuracy"]),
          na.rm = TRUE
        ),
        NetBenefit = mean(
          sapply(metrics, function(x) x["net_benefit"]),
          na.rm = TRUE
        )
      )
    )

    # Guardar variables seleccionades
    vars_seleccionades_step[[i]] <- vars_selec_bin
  }
})
...
save(
  resultats_step50,
  vars_seleccionades_step,
  file = "resultats_step50.RData"
)
```
